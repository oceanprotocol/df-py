...
@enforce_types
def saveSymbolsCsv(symbols_token_addrs_at_chain: List[str], csv_dir: str, chainID: int):
    """
    @description
      Save the symbols tokens for this chain

    @arguments
      symbols_token_addrs_at_chain -- 
      csv_dir -- directory that holds csv files
      chainID -- which chain (network)
    ...
    with open(csv_file, "w") as f:
        writer = csv.writer(f)
        writer.writerow(["chainID", "token_addr"])
        for token_addr in symbols_token_addrs_at_chain:
            assertIsEthAddr(token_addr)
            row = [chainID, 
                   token_addr.lower()]
            writer.writerow(row)
...
@enforce_types
def loadSymbolsCsvs(csv_dir: str) -> Dict[int, List[str]]:
    """
    @description
      Load all symbols tokens across all chains

    @return
      symbols_token_addrs -- dict of [chainID] : list_of_addr
    """
    csv_files = symbolsCsvFilenames(csv_dir)
    symbols_token_addrs: dict = {} 
    for csv_file in csv_files:
        chainID = chainIDforSymbolsCsv(csv_file)
        symbols_token_addrs[chainID] = loadSymbolsCsv(csv_dir, chainID)
        
    return symbols_token_addrs


@enforce_types
def loadSymbolsCsv(csv_dir: str, chainID: int) -> List[str]:
    """
    @description
      Load symbols tokens for this chainID

    @return
      symbols_token_addrs_at_chain -- 
    """
    csv_file = symbolsCsvFilename(csv_dir, chainID)
    symbols_token_addrs_at_chain: List[str] = []
    with open(csv_file, "r") as f:
        reader = csv.reader(f)
        for row_i, row in enumerate(reader):
            if row_i == 0:  # header
            ...
            symbols_token_addrs_at_chain.append(token_addr)

    print(f"Loaded {csv_file}")
    return symbols_token_addrs_at_chain
