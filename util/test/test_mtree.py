#from https://github.com/raiden-network/raiden/blob/e43ebbb8c09407d9793e50b1005bfdb67c267b4a/raiden/tests/unit/test_mtree.py

from hashlib import sha256
import pytest

from util.exceptions import HashLengthNot32
from util.merkle_tree import (
    MERKLEROOT,
    compute_layers,
...
def test_compute_layers_duplicated():
    hash_0 = sha256(b'x') #or sha256(b'x').digest() ??
    hash_1 = sha256(b'y') #"", and all below too??
...
def test_two():
    ...
    assert root == sha256(hash_0 + hash_1)
...
def test_three():
    ...
    assert sha256(hash_0 + hash_1) == hash_01
    calculated_root = sha256(hash_2 + hash_01)
    ...
    assert proof2 == [sha256(hash_0 + hash_1)]
    assert root == calculated_root
