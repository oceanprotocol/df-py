...
def test_calcDID():
    ...
    for [did,chain_id,address] in data:
        ...
        assert calcDID(address.lower(),int(chain_id)) == did # address is not case sensitive
        assert calcDID(address.upper(),int(chain_id)) == did # address is not case sensitive
