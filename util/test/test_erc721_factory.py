...
def test_direct():  # pylint: disable=too-many-statements
    ...
    OCEANtoken = BROWNIE_PROJECT.MockOcean.deploy(address0, {"from": GOD_ACCOUNT})
    OCEAN_address = OCEANtoken.address

    # God deploys templates
    erc721_template = BROWNIE_PROJECT.ERC721Template.deploy({"from": GOD_ACCOUNT})
    erc20_template = BROWNIE_PROJECT.ERC20Template.deploy({"from": GOD_ACCOUNT})
    pool_template = BROWNIE_PROJECT.BPool.deploy({"from": GOD_ACCOUNT})
    ...
    erc721_factory = BROWNIE_PROJECT.ERC721Factory.deploy(
    ...
    dataNFT1 = BROWNIE_PROJECT.ERC721Template.at(dataNFT1_address)
    ...
    ss_bot = BROWNIE_PROJECT.SideStaking.deploy(
