...
def test_simple():
    ...
    # test helper - just C1
    rewards_per_lp, rewards_info = calc_rewards_tst_C1(stakes, nftvols, OCEAN_avail)
...
def test_two_LPs__one_NFT__one_LP_created():
    ...
    OCEAN_avail = 10.0
    rewards_per_lp, rewards_info = calc_rewards_tst_C1(
...
def test_mix_upper_and_lower_case():
    ...
    rewards_per_lp, _ = calc_rewards_tst(stakes2b, stakes2b, nftvols, OCEAN_avail)
    assert target_rewards_per_lp == rewards_per_lp
    assert target_rewards_info == rewards_info

    rewards_per_lp, _ = calc_rewards_tst(stakes2c, stakes2c, nftvols, OCEAN_avail)
    assert target_rewards_per_lp == rewards_per_lp
    assert target_rewards_info == rewards_info

    rewards_per_lp, _ = calc_rewards_tst(stakes, stakes, nftvols2a, OCEAN_avail)
    assert target_rewards_per_lp == rewards_per_lp
    assert target_rewards_info == rewards_info

    rewards_per_lp, _ = calc_rewards_tst(stakes, stakes, nftvols2b, OCEAN_avail)
    assert target_rewards_per_lp == rewards_per_lp
    assert target_rewards_info == rewards_info

    rewards_per_lp, _ = calc_rewards_tst(
...
def testcalc_rewards_tst_math():
    ...
    rewards_per_lp, _ = calc_rewards_tst_C1(stakes, nftvols, OCEAN_avail)
