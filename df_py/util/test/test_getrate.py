...
def test_getBinanceRate_OCEAN_sameday():
    r = get_rate.getBinanceRate("OCEAN", "2022-01-20", "2022-01-20")
    assert r == approx(0.75, 0.1)


@enforce_types
def test_getCoingeckoRate_OCEAN_sameday():
    r = get_rate.getCoingeckoRate("OCEAN", "2022-01-20", "2022-01-20")
    assert r == approx(0.75, 0.1)
...
def test_get_rate_H2O():
    ...
    r = get_rate.getCoingeckoRate("H2O", "2022-05-13", "2022-05-25")
    assert r == 1.618


@enforce_types
def test_get_rate_fallback():
    with patch("df_py.util.get_rate.getBinanceRate") as mock1:
    ...
    with patch("df_py.util.get_rate.getBinanceRate") as mock1:
        mock1.return_value = None
        with patch("df_py.util.get_rate.getCoingeckoRate") as mock2:
...
def test_getCoingeckoRate_empty():
    ...
    with pytest.raises(ValueError):
        with patch("df_py.util.get_rate._coingeckoId") as mock1:
            mock1.return_value = ""
            r = get_rate.getCoingeckoRate("OCEAN", "2022-01-20", "2022-01-20")

    with patch("df_py.util.get_rate.json.load") as mock1:
        mock1.return_value = {}
        r = get_rate._coingeckoId("OCEAN")
