...
from df_py.web3util.oceanutil import (
    ...
    get_zero_consume_mkt_fee_tuple,
    OCEAN_token,
    set_allocation,
    veAllocate,
    veOCEAN,
...
def consume_DT(DT, pub_account, consume_account):
    ...
    provider_fee = get_zero_provider_fee_tuple(w3, pub_account)

    consume_mkt_fee = get_zero_consume_mkt_fee_tuple()
    DT.startOrder(
        consume_account,
        service_index,
        provider_fee,
...
def random_create_dataNFT_with_FREs(web3: Web3, num_FRE: int, base_token):
    ...
    for account in accounts:
        send_ether(web3, get_account0(), account.address, to_wei(1))
    ...
    for FRE_i in range(num_FRE):
        ...
        (data_NFT, DT, exchangeId) = create_data_nft_with_fre(
            web3, accounts[account_i], base_token
        )
        assert FixedPrice(web3.eth.chain_id).isActive(exchangeId) is True
...
def buy_DT_FRE(
    ...
    base_token.approve(
        FixedPrice(chain_id).address,
        to_wei(max_TOKEN),
        {"from": from_account},
    )

    feesInfo = FixedPrice(chain_id).getFeesInfo(exchangeId)
    FixedPrice(chain_id).buyDT(
        exchangeId,
        to_wei(DT_buy_amt),
        to_wei(max_TOKEN),
        feesInfo[1],
